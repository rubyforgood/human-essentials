# Request Mailer

This is an example of the email Human Essentials sends in this situation.  Names and addresses are made up.

From: <no-reply@humanessentials.app>

To: <director@communityoutreach.org>, <requester@communityoutreach.org>

(This email goes to the Partner's main email address and to the Partner user who submitted the Request.)
 
Subject: Your essentials request (#1042) has been canceled.

Hello Community Outreach Program,

Your essentials request (#1042) with Community Essentials Bank has been canceled.

Canceled Items Summary:
| Item Name   | Quantity | 
| --- | --- | 
| Disposable Diapers (Size 4) | 500 | 
| Disposable Diapers (Size 5) | 350 | 
| Pull-ups (2T–3T) | 200 | 
| Wipes (Unscented, Packs) | 120 | 

If you believe this cancellation was made in error or you need to submit a replacement request, please reach out to Community Essentials Bank directly or visit the partner portal.

