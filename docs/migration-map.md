...
| A fifth, arriving from `main` on 2026-09-09 | `reminder_toggle_controller` (#5668) toggled `d-none` too, so the reminder schedule and reminder email text would never have hidden. Caught on merge and fixed to `hidden`. **This is the recurring cost of the class being gone rather than redefined**: nothing upstream knows, so every merge can bring another one. **`undefined-classes.py` does catch it** — its JavaScript arm reads `classList.toggle` and reports a class the stylesheet does not define, verified by planting `d-none` back into that controller on 2026-09-09 and watching it fire. An earlier version of this row said the opposite; it was written before the check was tested and was wrong. So the guard already exists: **run `undefined-classes.py` after any merge from `main`**, and it will name the file. |
...
| Native `window.confirm` on all 44 destructive actions | Browser chrome — unstyled, unbranded, and it announces the page's hostname above the message. **Three audits could not see it and one depended on it**: `overlay-audit.js` opens `<dialog>` elements and a native confirm is not in the DOM; axe scans the document and has nothing to scan; and the system specs drive it with Capybara's `accept_confirm`, which only works on a *native* dialog, so a green suite is evidence for it. `overlay-audit.js` listens for the browser's `dialog` event now and reports 2. **Converted in the row below**, 35 spec call sites and all. And still not enough on its own: the specs only drive the confirmations they exercise, so when the whole mechanism was rolled out of the working tree in August 2026 the call sites without a spec went back to the browser's box in silence. `bin/design/confirm-audit.js` now **presses all 59 and dismisses them**, which is the only check that sees a confirmation at all. |
...
pw bin/design/confirm-audit.js    # presses every data-confirm and dismisses it -- 59 of them
pw bin/design/icon-audit.js       # every button's glyph, against bin/design/icon-lexicon.json
