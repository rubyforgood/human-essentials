...
| A fifth, arriving from `main` on 2026-09-09 | `reminder_toggle_controller` (#5668) toggled `d-none` too, so the reminder schedule and reminder email text would never have hidden. Caught on merge and fixed to `hidden`. **This is the recurring cost of the class being gone rather than redefined**: nothing upstream knows, so every merge can bring another one. `undefined-classes.py` does not catch it — the class is in a `.js` file, not a class attribute — but `d-none` in a `classList.toggle` is worth grepping for after any merge from `main`. |
