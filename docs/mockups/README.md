...
| `table-stacking.html` | Every table scrolls sideways at every width — 15 of 15 at 320px, the worst hiding 80%. Should a table stack into labelled fields when its container is narrow? | **Awaiting a decision.** Recommends B: the leading fields with the rest behind a per-row disclosure, switched by a **container** query rather than a viewport one — measured, `/purchases` loses 271px of container going from 1023 to 1024 when the sidebar appears. Records that changing `display` on a table drops its semantics, so the roles have to be restored explicitly. |
