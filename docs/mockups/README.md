...
| `showing-line-options.html` | `#date_range_label` produces a sentence fragment that nothing reads. Where should the sentence go, and is the fragment fit to build on? | Awaiting a decision. Records that 4 of the helper's 13 outputs are wrong, ambiguous or empty. |
