...
| `page-header-actions.html` | `/requests` carries four page actions where design.md allows three. Where do the extra ones go, and what is "Calculate product totals" for? | **Awaiting a decision.** Recommends B: one Export menu for the CSV and the picklist PDF, and the totals summary moved onto the card whose rows it totals. Records that the rule had been written for weeks with **nothing enforcing it** — `bin/design/button-audit.js` counts now, and found this is the only page in the app that breaks it. |
