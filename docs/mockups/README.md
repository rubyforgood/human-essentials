...
| `remove-control-and-empty-state.html` | Should the icon-only remove control get a tooltip or become a labelled ghost button, and what does the card look like with no rows? | **Awaiting a decision.** Recommends the labelled ghost button, on consistency: four of five `remove_element_button` call sites already render it, including the partner request form, which is the same shape. Recommends the `:cold_start` empty state, which the card has never had. |
