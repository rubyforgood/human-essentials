...
| `line-item-divider-options.html` | The line item card looks busy. Is a divider between every row industry standard, and what is the alternative? | **Awaiting a decision.** Recommends option B: no divider between rows, keeping the three that separate bands. Measured 7 card-wide rules against 4 with them gone. |
