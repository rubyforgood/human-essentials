...
| `clipped-cell-tooltip.html` | Can a clipped `.notes` cell reveal its full text on hover, and what is standard for that? | **Awaiting a decision.** Recommends a real tooltip on hover *and* focus, only on cells whose text is actually clipped. Records that this is the one place a tooltip is the standard answer, against three earlier refusals that were about naming a control. |
