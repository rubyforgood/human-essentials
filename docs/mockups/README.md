...
| `scan-field-width-options.html` | The barcode field lines up with nothing. Should it match the Storage location field, be half the card, or stay sized to its content? | **Awaiting a decision.** Recommends option A: the scan bar joins the same grid the cards above use and takes column one. Recorded that a fixed 240px is 33px *wider* than the column at 1024px. |
