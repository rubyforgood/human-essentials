...
| `table-scroll-affordance.html` | 486px of columns are off screen and nothing says so. What signals that a table scrolls sideways? | **Awaiting a decision.** Recommends a directional fade at the overflowing edge, which works everywhere, plus scrollbar styling as a platform-dependent bonus — `::-webkit-scrollbar` takes 0px in headless Chromium even with overlay scrollbars disabled, and Firefox ignores it. |
