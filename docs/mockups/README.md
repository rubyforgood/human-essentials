...
| `filter-consistency-options.html` | Two bugs (every modal opens top-left; Calculate product totals is inside the filter bar), plus: should the disclosure threshold go, how does industry take a custom date range, and are two Clears needed? | Awaiting a decision. Records that Tailwind's preflight `margin: 0` breaks native modal centring in 28 files, and that Custom turns a 64px cell into 216px. |
