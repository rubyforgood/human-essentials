...
| `filter-density-options.html` | The date range cell is twice the width of its neighbours and the actions take a row of their own. Should a dense filter set collapse? | Awaiting a decision. Two defects measured and fixed in the mockup (4 rows to 3 on `/donations`), plus three densities. |
