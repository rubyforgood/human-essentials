...
| `table-edge-weight.html` | The grey edge on a scrolling table looks heavy and dated. How strong should it be? | **Awaiting a decision.** Recommends B, Ant Design's production value of 6% over 10px — a sixth of what shipped, which tints **28.5% of the visible table**. Records that I chose the heavy one on a **bad metric**: a strip-mean rewards a broad smear and scored a crisp 1px rule 1.83, "imperceptible", which was wrong. |
