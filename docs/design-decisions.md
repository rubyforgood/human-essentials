...
## 2026-09-09 — Measure what is closed, not what is growing

`status.rb` reported "347 of 384 views carrying design system markup", and that figure was
republished in the change log's generated table. Asked what remained of the migration, I read the
files it named.

**Every one of them was already migrated. The check was wrong six times out of six.**

- Three were bare `f.input` — which is the *finished* state, not an unmigrated one, because
  `config.default_wrapper = :essentials` means the wrapper supplies the wrapper div, the label, the
  control classes and the error. Each of those three files carries a comment explaining that
  hand-pasted classes were *removed* because the wrapper does it.
- One used a data-table cell class (`class="percent"`), one a Stimulus controller
  (`data-controller="highchart"`), one a helper call whose argument was a variable
  (`source.association association_field`) rather than the symbol literal the pattern wanted.

### Why widening it again was the wrong instinct

I widened it. Adding the page wrapper, `link-brand`, `EssentialsUiHelper` and the cell conventions
took 347 to 360; resolving "renders a migrated partial", which the check's own comment already
claimed it did, took it to 364; adding simple_form builder calls took it to 346 of 349. Each pass
recovered real work. And the last three files would have needed a fourth pass.

The check's own comment records that the pattern had already been widened once before, for 51
pages, for exactly the same reason. **That is the shape of a check that cannot be finished.** The
design system gains vocabulary with every component, so a positive-marker test is permanently one
component behind — and every miss reports completed work as outstanding, which is the direction
that makes a number misleading rather than merely imprecise.

### The inversion

The legacy vocabulary is *closed*. ADR 0011 deleted Bootstrap and AdminLTE; they cannot gain new
words. So the primary measure asks the question with a stable answer:

> Does this view still use the old system?

which is what "is the migration done" means. It reads **0 of 350**.

Rejected: keeping the positive count as the headline and accepting that it undercounts. It is still
printed, because it is a useful coverage indicator, but labelled as an indicator with the reason
attached — a four-line table row can be fully migrated and have nothing to mark.

Also rejected: dropping the files that cannot match. The components in `shared/essentials/` cannot
carry a marker for the system they *are*, and gem and framework partials are not ours to migrate —
but silently excluding them reads the same as overlooking them, so they are counted and named
separately.

### The first draft of the inverted check was wrong too

It reported three views as legacy: `admin/dashboard`, `partners/_show_header` and `reports/index`.
All three carry `card-surface`, which is **ours** — and `\bcard\b` matches the "card" inside it,
because the word boundary sits before the hyphen. Bootstrap's card suffixes are enumerated now.
`float-right` came out as well: it is a Tailwind utility as much as a Bootstrap one, so its
presence says nothing about which system a view is on.
