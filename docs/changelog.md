...
| `PENDING` | **The pager says how many rows matched.** "Page 3 of 19" became "Showing 31–45 of 272 requests" — option A of four designs put up in `docs/mockups/pagination-designs.html`. A page number is a proxy: it changes meaning whenever the page size does, which the three bands had just made concrete, and it never answered the question an index page's filter bar raises. The noun comes from Kaminari's `entry_name`, lowercased word by word so a word with an internal capital keeps it and a future `/admin/ndbn_members` reads "NDBN members". `« First` and `Last »` were kept although the mockup's A panel dropped them — jumping to the oldest record is a real task on audits and events. A rows-per-page select and load-more were both argued against in the mockup and not built. |
