RSpec.describe "Product Drive Participant", type: :system, js: true, skip_seed: true do
  before do
    sign_in(@user)
  end
  ...
  context "When a user views the index page" do
    ...
    context "When the Product Drives have donations associated with them already" do
      before(:each) do
      ...
      it "shows existing Product Drive Participants in the #index with some summary stats" do
        visit subject
      ...
      it "allows single Product Drive Participants to show semi-detailed stats about donations from that product drive" do
