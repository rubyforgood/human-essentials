RSpec.describe "Organization management", type: :system, js: true do
  ...
  shared_examples "organization role management checks" do |user_factory|
    ...
    it 'can remove that user from the organization' do
      ...
      accept_confirm_dialog do
        click_row_action "Remove user", row: managed_user.preferred_name
      end
    ...
    it "can promote that user from the organization" do
      visit organization_path
      accept_confirm_dialog do
        click_row_action "Promote to admin", row: managed_user.preferred_name
      end
    ...
    it "can demote that user from the organization" do
      ...
      visit organization_path
      accept_confirm_dialog do
        click_row_action "Demote to user", row: managed_user.preferred_name
      end
  ...
  context "while signed in as an organization admin" do
    ...
    describe "Editing the organization" do
      ...
      # From main's #5668. HEAD's copy of the consistency test was dropped here rather
      # than kept: main moved that test into the "with monthly deadline reminders enabled"
      # context below, so keeping both would have run it twice.
      it "shows the reminder schedule fields only when monthly deadline reminders are enabled" do
        ...
        expect(page).not_to have_content("Additional text for the reminder email")

        choose_deadline_reminders_yes
        expect(page).to have_field("Deadline day in reminder email", visible: :visible)
        expect(page).to have_content("Additional text for the reminder email")
