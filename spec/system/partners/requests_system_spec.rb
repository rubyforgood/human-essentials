RSpec.describe "Partners profile served area behaviour", type: :system, js: true do
  ...
  describe "GET #index" do
    ...
    # `enable_packs` is gone (main's #5251), so these are no longer behind a flag and the
    # "with packs off" context has been dropped: it asserted the unit select was absent,
    # which is now the opposite of what ships.
    it "should require a unit selection" do
      ...
      expect(page).to have_text "Items that come in packs also need a unit chosen."
      expect(Request.count).to eq(0)
    end
