...
RSpec.describe BroadcastAnnouncement, type: :model, seed_items: false do
  ...
  context "filter_announcements" do
    ...
    it "should include only announcements from the passed organization" do
      create(:broadcast_announcement, message: "test", user_id:, organization_id:)
      create(:broadcast_announcement, message: "test", user_id:, organization_id:)
      create(:broadcast_announcement, message: "test", user_id:)
      create(:broadcast_announcement, message: "test", user_id:, organization_id: create(:organization, skip_items: true).id)
    ...
    it "shouldn't include expired announcements" do
      create(:broadcast_announcement, message: "test", user_id:, organization_id:)
      create(:broadcast_announcement, message: "test", user_id:, expiry: 2.days.ago, organization_id:)
      create(:broadcast_announcement, message: "test", user_id:, expiry: 5.days.ago, organization_id:)
      create(:broadcast_announcement, message: "test", user_id:, expiry: Time.zone.today, organization_id:)
      expect(BroadcastAnnouncement.filter_announcements(organization_id).count).to eq(2)
    end

    it "sorts announcements from most recently created to last" do
      announcement_1 = create(:broadcast_announcement, message: "test", user_id:, organization_id:, created_at: Time.zone.today)
      announcement_2 = create(:broadcast_announcement, message: "test", user_id:, organization_id:, created_at: 2.days.ago)
      announcement_3 = create(:broadcast_announcement, message: "test", user_id:, organization_id:, expiry: 1.day.ago, created_at: 3.days.ago)
      announcement_4 = create(:broadcast_announcement, message: "test", user_id:, organization_id:, created_at: 4.days.ago)
