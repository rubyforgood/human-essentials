  ...
  describe '#generate_csv' do
    ...
    let(:distribution_items_and_quantities) {
      [
        [
          [duplicate_item, 5],
          [create(:item, name: "A Item", value_in_cents: 1000, organization: organization, package_size: 7), 7],
          [duplicate_item, 3]
        ],
    ...
    context 'while "Include packages in distribution export" is set to yes' do
      ...
      it 'should match the expected content with package count of each item for the csv' do
        csv = <<~CSV
          ...
          #{partner.name},04/04/2025,04/04/2025,#{storage_location.name},8,24.0,shipped,$15.01,scheduled,"",comment 0,7,1.0,0,0,0,0,8,4.0,0,0
    ...
    context 'while both in-kind values and package count are enabled for export' do
      ...
      it 'should match the expected content with in-kind value and package count of each item for the csv' do
        csv = <<~CSV
          ...
          #{partner.name},04/04/2025,04/04/2025,#{storage_location.name},8,24.0,shipped,$15.01,scheduled,"",comment 0,7,70.00,1.0,0,0.00,0,0,0.00,0,8,24.00,4.0,0,0.00,0
