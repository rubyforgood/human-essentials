  ...
  describe "#deallocate" do
    ...
    context "when inventory items are available" do
      ...
      it "deallocates items" do
        before_deallocate_1 = item_inventory1.quantity
        before_deallocate_2 = item_inventory2.quantity

        service = DeallocateKitInventoryService.new(kit, storage_location).deallocate

        expect(service.error).to be_nil
        expect(item_inventory1.reload.quantity).to eq(before_deallocate_1 + quantity_of_items1)
        expect(item_inventory2.reload.quantity).to eq(before_deallocate_2 + quantity_of_items2)
      end
    end
    ...
    context "when inventory item doesn't exist" do
      ...
      it "returns error" do
        before_try_deallocate_1 = item_inventory1.quantity
        before_try_deallocate_2 = item_inventory2.quantity

        service = DeallocateKitInventoryService.new(kit, storage_location).deallocate

        expect(service.error).to include("Couldn't find Item with")
        expect(item_inventory1.reload.quantity).to eq(before_try_deallocate_1)
        expect(item_inventory2.reload.quantity).to eq(before_try_deallocate_2)
      end
    end
  end
end
