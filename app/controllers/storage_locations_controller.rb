class StorageLocationsController < ApplicationController
  ...
  def show
    ...
    @items_out = LineItem
                 .joins("
LEFT OUTER JOIN distributions ON distributions.id = line_items.itemizable_id AND
line_items.itemizable_type = 'Distribution'
LEFT OUTER JOIN items ON items.id = line_items.item_id
LEFT OUTER JOIN adjustments ON adjustments.id = line_items.itemizable_id AND
line_items.itemizable_type = 'Adjustment'
LEFT OUTER JOIN transfers ON transfers.id = line_items.itemizable_id AND
line_items.itemizable_type = 'Transfer'")
                 .where("(distributions.storage_location_id = :id or
                         (adjustments.storage_location_id= :id and line_items.quantity < 0) or
                         transfers.from_id = :id) and
                         items.organization_id= :organisation_id",
                        id: params[:id],
                        organisation_id: current_organization.id)
                 .select("sum( case when line_items.quantity < 0
                         then -1*line_items.quantity else line_items.quantity END ) as quantity,
                         items.id, items.name")
...
line_items.itemizable_type = 'Transfer'")
                       .where("(distributions.storage_location_id = :id or
                              ...
                              id: params[:id],
                              organisation_id: current_organization.id)
                       .sum('case when line_items.quantity < 0 then -1*line_items.quantity
                             else line_items.quantity END')
    @items_in = LineItem
                .joins("
LEFT OUTER JOIN donations ON donations.id = line_items.itemizable_id AND
line_items.itemizable_type = 'Donation'
LEFT OUTER JOIN purchases ON purchases.id = line_items.itemizable_id AND
line_items.itemizable_type = 'Purchase'
LEFT OUTER JOIN items ON items.id = line_items.item_id
LEFT OUTER JOIN adjustments ON adjustments.id = line_items.itemizable_id AND
line_items.itemizable_type = 'Adjustment'
LEFT OUTER JOIN transfers ON transfers.id = line_items.itemizable_id AND
line_items.itemizable_type = 'Transfer'")
                .where("(donations.storage_location_id = :id or
                        purchases.storage_location_id = :id or
                        (adjustments.storage_location_id = :id and line_items.quantity > 0) or
                        transfers.to_id = :id) and items.organization_id = :organisation_id",
                       id: params[:id],
                       organisation_id: current_organization.id)
...
line_items.itemizable_type = 'Transfer'")
  ...
  def inventory
    @storage_location = current_organization.storage_locations
