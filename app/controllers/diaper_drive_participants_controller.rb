class DiaperDriveParticipantsController < ApplicationController
  ...
  def show
    @diaper_drive_participant = current_organization.diaper_drive_participants
                                                    .includes(:donations)
                                                    .find(params[:id])
  end
  ...
  def update
    ...
    if @diaper_drive_participant.update_attributes(diaper_drive_participant_params)
      redirect_to diaper_drive_participants_path,
                  notice: "#{@diaper_drive_participant.name} updated!"
    else
      flash[:error] = "Something didn't work quite right -- try again?"
      render action: :edit
    end
  ...
  def import_csv
    if params[:file].nil?
      redirect_back(fallback_location:
                      diaper_drive_participants_path(organization_id: current_organization))
