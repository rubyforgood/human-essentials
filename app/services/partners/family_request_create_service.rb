module Partners
  class FamilyRequestCreateService
    ...
    def call
      ...
      request_create_svc = Partners::RequestCreateService.new(partner_user_id: partner_user_id, comments: comments, item_requests_attributes: item_requests_attributes)
    ...
    def item_requests_attributes
      @item_requests_attributes ||= family_requests_attributes.values.map do |fr_attr|
        item = included_items.find { |item| item.id == fr_attr[:item_id].to_i }
    ...
    def convert_person_count_to_item_quantity(item_id:, person_count:)
      item = included_items.find { |item| item.id == item_id.to_i }

      # Could not find matching item so return nil instead
      return nil if item.blank?

      person_count * item.default_quantity.abs
    end
