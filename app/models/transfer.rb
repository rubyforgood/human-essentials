...
class Transfer < ApplicationRecord
  ...
  def self.storage_locations_transferred_to_in(organization)
    includes(:to).where(organization_id: organization.id).distinct(:to_id).collect(&:to).uniq.sort_by{ |organization| organization.name}
  end

  def self.storage_locations_transferred_from_in(organization)
    includes(:from).where(organization_id: organization.id).distinct(:from_id).collect(&:from).uniq.sort_by{ |organization| organization.name}
  end
