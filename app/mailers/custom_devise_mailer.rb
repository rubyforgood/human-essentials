class CustomDeviseMailer < Devise::Mailer
  ...
  def subject_for(key)
    ...
    if resource.has_role?(:partner, :any) && resource.id == resource.partner.primary_user&.id
      "You've been invited to be a partner with #{resource.partner.organization.name}"
    elsif resource.has_role?(:partner, :any) && resource.id != resource.partner.primary_user&.id
