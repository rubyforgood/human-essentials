...
Human Essentials is one of many projects initiated and ran by Ruby for Good. You can find out more about Ruby for Good at https://rubyforgood.org
...
### 💬 Join us on slack
You can sign up [here](https://rubyforgood.slack.com/join/shared_invite/zt-1kfeimohe-KL~~~6Lkof7G94_7Ojd_Hw#/shared-invite/email) and find us in #human-essentials. Many helpful members are available to answer your questions. Just ask, and someone will be there to assist you!
...
### Install Postgres
This app uses PostgreSQL for all environments. Refer to the following links for installation instructions: [MacOSX](https://www.digitalocean.com/community/tutorials/how-to-use-postgresql-with-your-ruby-on-rails-application-on-macos), [Ubuntu](https://www.digitalocean.com/community/tutorials/how-to-use-postgresql-with-your-ruby-on-rails-application-on-ubuntu-18-04). Do you develop on Windows? We'd love to hear (and for you to submit a PR explaining) how you do it. 🙏🏻

### Run Setup
The following command should run all the neccessary steps to get up and running.
```
bin/setup
```
...
Run delayed jobs locally with the `rake jobs:work` command. This is necessary to view any emails, which should appear in your browser. Alternatively, you can run a specific delayed job by opening a Rails console and doing something like:
...
- The generated `schema.rb` file may include or omit `id: :serial` for `create table`, and `null: false` for `t.datetime`. According to Aaron, this can safely be ignored. It is recommended to commit the schema.rb file only if there are changes to the DB schema (e.g., migrations).
 ...
 *  Work on the task and ask questions on the corresponding issue if needed
